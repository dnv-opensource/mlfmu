...
class Variable(BaseModelConfig):
    name: str = Field(None, description="Unique name for the port.", examples=["windSpeed", "windDirection"])
    ...
    start_value: Optional[Union[float, str, bool, int]] = Field(
        0, description="Initial value of the signal at time step 1. Type should match the variable type."
    )
    ...
    length: Optional[int] = Field(
        None, description="Defines the number of entries in the signal if the signal is array.", examples=[3, 5]
    )


class InternalState(BaseModelConfig):
    agent_output_indexes: List[
        Annotated[str, StringConstraints(strip_whitespace=True, to_upper=True, pattern=r"^(\d+|\d+:\d+)$")]
    ] = Field(
        None,
...
class ModelComponent(BaseModelConfig):
    ...
    inputs: List[InputVariable] = Field(
        [], description="List of input signals of the simulation model.", examples=[[create_fmu_signal_example()]]
    )
    outputs: List[OutputVariable] = Field(
        [], description="List of output signals of the simulation model.", examples=[[create_fmu_signal_example()]]
    )
    parameters: List[InputVariable] = Field(
        [], description="List of parameter signals of the simulation model.", examples=[[create_fmu_signal_example()]]
    )
    ...
    uses_time: Optional[bool] = Field(
        False, description="Whether the agent consumes time data from co-simulation algorithm."
    )
...
class FmiModel:
    ...
    def add_variable_references(
        self, inputs: List[InputVariable], parameters: List[InputVariable], outputs: List[OutputVariable]
    ):
        ...
        for var in inputs:
            ...
            fmi_variable = FmiInputVariable(
                causality=FmiCausality.INPUT, variable_references=var_port_refs, **var.__dict__
            )
        ...
        for var in outputs:
            ...
            fmi_variable = FmiOutputVariable(
                causality=FmiCausality.OUTPUT, variable_references=var_port_refs, **var.__dict__
            )
    ...
    def get_template_mapping(self) -> Tuple[List[Tuple[int, int]], List[Tuple[int, int]]]:
    ...
    def get_total_variable_number(self) -> int:
        ...
        num_variables = reduce(lambda prev, current: prev + len(current.variable_references), all_fmi_variables, 0)
