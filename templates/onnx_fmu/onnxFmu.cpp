...
bool  OnnxFmu::RunOnnxModel(cppfmu::FMIReal currentCommunicationPoint, cppfmu::FMIReal dt) {
    ...
    {   
        ...
        int numInputs = 1;
        inputs.emplace_back(Ort::Value::CreateTensor<float>(memoryInfo, onnxInputs_.data(), onnxInputs_.size(), inputShape_.data(), inputShape_.size()));

        if (NUM_ONNX_STATES > 0){
            inputs.emplace_back(Ort::Value::CreateTensor<float>(memoryInfo, onnxStates_.data(), onnxStates_.size(), stateShape_.data(), stateShape_.size()));
        ...
        if (ONNX_USE_TIME_INPUT) {
            ...
            onnxTimeInput_[1] = dt;
            inputs.emplace_back(Ort::Value::CreateTensor<float>(memoryInfo, onnxTimeInput_.data(), onnxTimeInput_.size(), timeInputShape_.data(), timeInputShape_.size()));
...
bool  OnnxFmu::DoStep(cppfmu::FMIReal currentCommunicationPoint, cppfmu::FMIReal dt, cppfmu::FMIBoolean /*newStep*/, cppfmu::FMIReal& /*endOfStep*/)
{
