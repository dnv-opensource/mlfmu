...
union FMIVariable
{
    struct {cppfmu::FMIReal     real;};
    struct {cppfmu::FMIInteger  integer;};
    struct {cppfmu::FMIString   string;};
    struct {cppfmu::FMIBoolean  boolean;};
};
...
private:
    ...
    std::array<std::array<int, 2>, NUM_ONNX_FMU_OUTPUTS> onnxOutputValueReferenceIndexPairs_{ONNX_OUTPUT_VALUE_REFERENCES};
